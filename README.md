...
比如，在编译 Linux 内核的时候，除了内核镜像 `zImage` 之外会同时生成设备树文件 `vexpress-v2p-ca9.dtb`，也需要传递给 QEMU，否则启动后是黑屏。我不确定实验指导书中的“修改源代码”是不是用来解决这个问题的（不管是不是我都不建议……尤其是当你不知道所改的东西会带来的具体影响之时）。
